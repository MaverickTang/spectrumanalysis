...
%             anomalyData(innovativeFrequencies1(j)+k) = anomalyData(innovativeFrequencies1(j)+k)+innovativeAnomalyMagnitude(1);
...
figure;
subplot(1,2,1);
plot(1:6300, subspectrumData(:, 5)); % Convert to decibels
title('Original Data');
xlabel('Time Index');
ylabel('Intensity (dB)');
...
subplot(1,2,2);
plot(1:6300, anomalyData(:, 1)); % Convert to decibels
hold on;

% Calculate the difference and find significant deviations
difference = abs(anomalyData(:, 1) - subspectrumData(:, 5));
threshold = 3; % Define a threshold for significant deviation
significantDeviationIndices = find(difference > threshold);

% Highlight the significant deviations
plot(significantDeviationIndices, anomalyData(significantDeviationIndices, 1), 'r*', 'MarkerSize', 5);

% Connect the highlighted points with a line if they are consecutive
if ~isempty(significantDeviationIndices)
    % Extract the consecutive segments of indices
    breaks = find(diff(significantDeviationIndices) > 1);
    startPoints = [1; breaks+1];
    endPoints = [breaks; length(significantDeviationIndices)];

    for i = 1:length(startPoints)
        % Plot each segment as a connected line
        segmentIndices = significantDeviationIndices(startPoints(i):endPoints(i));
        plot(segmentIndices, anomalyData(segmentIndices, 1), 'r-', 'LineWidth', 2);
    end
end
...
xlabel('Time Index');
ylabel('Intensity (dB)');
legend('Anomaly Data', 'Significant Deviations');
hold off;
